...
## Detecting ATBI following LTBI without vaccination
...
Briefly described, we calculate the probability that a person gets LTBI in year $t$ and subsequently ATBI in year $y_i$, then we sum these probabilities for $t=1, \dots, y_i$.

The survival function can thus be defined as:

$$S(y_i | p_L, p_A) = 1 - \Sigma_{t=1}^{y_i} f(t | p_L, p_A) $$
  ...
  
  {
    ...
    for (n in 1:N){
      ...
      if( ATBIstatus[n]==1 ){
        ...
        target += replicates*log(prob_atbi_in_year);
      } else {
        survival=0;
